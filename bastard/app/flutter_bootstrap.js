...
// Flutter's auto-generated bootstrap registers `flutter_service_worker.js`,
// which in current Flutter is a deprecated "tombstone" stub that
// unregisters itself and force-navigates clients on activate. For installed
// PWAs that produced a blank screen on launch because the stub provides no
// `fetch` handler and the forced navigation races CanvasKit / asset fetches.
//
// By providing this file in `web/`, Flutter skips generating its default
// bootstrap and uses ours, which intentionally does NOT pass
// `serviceWorkerSettings` to the loader. No service worker is registered,
// so no tombstone-and-reload cycle, and no blank screen.
//
// Trade-off: the app no longer works offline. If offline support is
// required, replace this with a real service worker (e.g. Workbox)
// instead of re-enabling Flutter's deprecated one.
